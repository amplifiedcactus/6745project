...
//structs for and gate and xor gate
struct andgate {
    int num; //number to identify gate
    int a;
    int b;
    int out;
    bool an; //true if a is negated
    bool bn; //true if b is negated
};
struct xorgate {
    int a;
    int b;
...
//function to remove gate from vector of andgates by num
...
//function to extract xor gates from andgates
void extract_xor_gates(std::vector<andgate>& andgates, std::vector<xorgate>& xorgates) {
    ...
    for (auto& gate : andgates) {
        for (auto& gate2 : andgates) {
            ...
            if (gate.num == gate2.num) {
                ...
                //find two gates that are candidates for xor
                if ((gate.a == gate2.a && gate.b == gate2.b && gate.an != gate2.an && gate.bn != gate2.bn) || (gate.a == gate2.b && gate.b == gate2.a && gate.an == gate2.an && gate.bn == gate2.bn)) {
                    ...
                    //then add xor gate or xnor gate, xor gate output is gate3.out, inputs are gate.a and gate.b
                    for (auto& gate3 : andgates) {
                        if (gate3.a == gate.out && gate3.b == gate2.out && gate3.an && gate3.bn) {
                            ...
                            int tempnum[] = {gate.num, gate2.num, gate3.num}; //need this to avoid modifying vector while iterating
...
int main(int argc, char *argv[]) {
    ...
    int buffer; //buffer for reading integers from file
    char newline_char; //buffer for reading newline character
    static unsigned M, I, L, O, A; //AAG header variables
    //read AAG header
    ...
    //create vectors for and gates and xor gates
    std::vector<andgate> andgates;
    std::vector<xorgate> xorgates;
    

    //Read in and gates
    int num = 0; //variable to keep track of number of and gates
    while (1) {
        int buffer2[3];
    ...
    //change to zero to disable xor extration
    if (1) {
        printf("before XOR extraction:\n");
        for (auto& gate : andgates)
            printf("and gate: out=%d a=%d (an=%d) b=%d (bn=%d) (num=%d)\n", gate.out, gate.a, gate.an, gate.b, gate.bn, gate.num);
        
        extract_xor_gates(andgates, xorgates);
        
        printf("After XOR extraction:\n");
        for (auto& gate : andgates)
    ...
    //add polynomials for outputs
    for (auto& out : outputs) {
        printf("output before inversion check: %d\n", out);
        if (out & 1)
            addpolyinv(polynomials, polycount, maxvar, out);
    ...
    //needed variables:
    //inputs - string containing all input variables
    //outputstring - string containing all output variables
    ...
    std::string order = "";
    //int i = maxvar;
    //Create a combined string of inputs and outputs
    ...
    //add inputs and outputs to removal set
    while(std::getline(ss, segment, ',')) {
        // Trim whitespace (so " x_2" becomes "x_2")
    ...
    //build the 'order' string by iterating from maxvar down to 0
    // and only adding variables not in the removal set
    for (int k = maxvar; k >= 0; k--) {
        // Construct the current token (e.g., "x_1")
